...
export default function LoginPage() {
  ...
  // Removendo completamente a função handleLogin duplicada
  // Esta função não está sendo usada e pode ser removida completamente
  const handleLogin = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if ((username === "admin" && password === "123456") || 
            (username === "administrador" && password === "123456789")) {
          localStorage.setItem('isLoggedIn', 'true');
        ...
        if (alunosSalvos) {
          const alunos = JSON.parse(alunosSalvos);
          const aluno = alunos.find(
            (a: any) => a.login === username && a.senha === password
          );
